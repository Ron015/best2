  ...
  module.exports = (client) => {
      client.manager
        .on("playerCreate", async (player) => {
            ...
            try {
              embed.setTitle(`<a:yes2:921988805442043925> **Joined** \`${client.channels.cache.get(player.voiceChannel).name}\``)
            } catch {}
            try {
              embed.setDescription(`**Commands bound to: ** <#${client.channels.cache.get(player.textChannel).id}>`)
              embed.addField(`${client.user.username} Powered by RôÑ#7322`)
            } catch {}
