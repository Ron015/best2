...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if ("756339292678389801" !== message.author.id)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(client.user.username, es.footericon)
        .setTitle(`${emoji.msg.ERROR}  Error | You are not allowed to run this command! Only the **\`BROKARONAGAMING#9999\`** is allowed to run this Cmd`)
      );
    if (!args[0])
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
